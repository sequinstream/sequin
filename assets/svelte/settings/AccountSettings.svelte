<script lang="ts">
  ...
  import { Trash, Cog, Plus, Ellipsis } from "lucide-svelte";
  ...
  function handleRevokeInvite(inviteId: string) {
    live.pushEventTo(`#${parent}`, "revoke_invite", {
      inviteId: inviteId,
  ...
  function handleInviteMember(event: SubmitEvent) {
    ...
    live.pushEventTo(`#${parent}`, "invite_user", {
        accountId: selectedAccount.id,
...
<div>
  ...
  <div class="flex flex-col gap-6 container w-auto">
              ...
              <Table.Cell>
                <div class="flex items-center gap-2">
                  ...
                  {#if user.id === currentUser.id}
                    <span class="text-xs text-muted bg-gray-200 rounded-full px-3 py-1">You</span>
                  {/if}
                </div>
              </Table.Cell>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild let:builder>
                    <Button variant="ghost" builders={[builder]}>
                      ...
                      <span class="sr-only">Account User Menu for {user.email}</span>
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem class="cursor-pointer flex gap-2 items-center">
                      <Trash class="size-4" />
                      Remove user
                    </DropdownMenuItem>
                  </DropdownMenuContent>
    ...
    <div class="bg-card text-card-foreground rounded-lg border shadow-sm">
      ...
      {#if !pendingInvites.length}
        <div class="flex flex-col gap-4 items-center text-center py-12 mx-auto my-auto px-6">
          ...
          <Button class="flex items-center gap-2" on:click={openInviteMemberDialog}>
            <Plus class="h-4 w-4" />
            Invite user
          </Button>
        </div>
                ...
                <Table.Cell class="flex justify-end">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild let:builder>
                      <Button variant="ghost" builders={[builder]}>
                        <Ellipsis class="h-4 w-4" />
                        <span class="sr-only">Invite Menu for {invite.sent_to}</span>
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem class="cursor-pointer flex gap-2 items-center" on:click={() => handleRevokeInvite(invite.id)}>
                        <Trash class="size-4" />
                        Revoke invite
                      </DropdownMenuItem>
                    </DropdownMenuContent>
