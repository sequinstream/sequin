...
if [[ -f "terraform-ecs-ec2/infra/backend.tfbackend" ]] || [[ -f "terraform-ecs-ec2/app/backend.tfbackend" ]]; then
...
cat > terraform-ecs-ec2/infra/backend.tfbackend << EOF
...
cat > terraform-ecs-ec2/app/backend.tfbackend << EOF
...
cat > terraform-ecs-ec2/app/remote-state.auto.tfvars << EOF
...
echo "   - terraform-ecs-ec2/infra/backend.tfbackend (key: infra/terraform.tfstate)"
echo "   - terraform-ecs-ec2/app/backend.tfbackend (key: app/terraform.tfstate)"
echo "   - terraform-ecs-ec2/app/remote-state.auto.tfvars (infra state reference)"
echo ""
echo "Next steps:"
echo "For infrastructure:"
echo "1. cd terraform-ecs-ec2/infra"
echo "2. terraform init -backend-config=backend.tfbackend"
echo ""
echo "For application:"
echo "1. cd terraform-ecs-ec2/app"
echo "2. terraform init -backend-config=backend.tfbackend"
echo ""
