...
We generally recommend using `pg_cron` if your write volume isn't too high (less than 1M writes per day). `pg_cron` is easier to setup. Otherwise, `pg_partman` is a more scalable solution.
...
- [Consume API](/consume/consume-api/overview)
- [Webhooks](/how-sequin-works#webhook-subscriptions)
- Sync API (coming soon)

Create a Sequence for your event table to begin streaming the changes to your applications.
