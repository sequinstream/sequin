...
You can pull messages from the consumer using Sequin's [HTTP API](/api/introduction) or [SDKs](/sdks). When a consumer is configured for pull consumption, Sequin will provision a unique endpoint for the consumer. Your workers will pull messages from this endpoint:
...
After [receiving a message](/api/http-pull-consumers/receive) from a pull consumer, your system will send a follow-up [acknowledgement request](/api/http-pull-consumers/ack) when the message has finished processing (i.e. `POST /consumers/:consumer_id/ack`).
