...
You can pull messages from the consumer using Sequin's [HTTP API](/management-api/introduction ) or [SDKs](/sdks). When a consumer is configured for pull consumption, Sequin will provision a unique endpoint for the consumer. Your workers will pull messages from this endpoint:
