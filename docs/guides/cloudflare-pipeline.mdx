...
By streaming every change in your database to a Cloudflare [Pipelines](https://developers.cloudflare.com/r2/pipelines/) using a [Sequin Webhook Sink](/reference/sinks/webhook), you can build a powerful, cost-effective data pipeline that captures all database activity in real-time. This approach comes with two key benefits:
...
<Steps titleSize="h3">
    <Step title="Create a new R2 bucket">
        Run the following command to create a new R2 bucketcalled `sequin-r2-bucket`:
...
<Steps titleSize="h3">
    ...
    <Step title="Transform the payload for the Cloudflare pipeline">
       The Cloudflare pipeline expects newline-delimited JSON with all the JSON objects wrapped in a single array. To achieve this, you'll need to add a transform to the sink.

       Skip down to the "Transform" section and click the **+ Create new transform** button.

       In the New Function page create name your function `cloudflare-pipeline-transform` with the type set to **Transform function**.
       ...
       This function will wrap the JSON object in a single array as required by the Cloudflare pipeline.
