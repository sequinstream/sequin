...
- An AWS account
- A Sequin account
- A Postgres database (Sequin works with any Postgres database version 12+ - including [RDS](/how-to/setup-rds)) with a basic `users` table containing an `id` column and a `name` column.
