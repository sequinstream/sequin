...
<Steps titleSize="h3">
    ...
    <Step title="Select source table">
        Select the table you want to capture changes from (i.e `public.posts`).
    </Step>
    ...
    <Step title="Choose message type">
        ...
        <Tip>
        In this example, you should choose "Rows" since you want to ensure every post gets an embedding and that embeddings are updated as posts change.
        </Tip>
    </Step>
    ...
    <Step title="Configure delivery settings">
        Under "Delivery configuration", choose a conservative value for "Request timeout" and set "Batch size" to 1 since we're just processing one post at a time.
    </Step>
