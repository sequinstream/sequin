...
With Sequin, you create [**sequences**](#sequences) which present a strictly ordered view of rows from one or more tables. Then, you can use this sequence to consume a table's rows by:
...
- `seq`: (internal) A Sequin sequence for the event entry.
