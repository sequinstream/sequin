...
description: "Create a reliable log of all changes in your database with rich contextual information"
---
...
Messages don't have a unique identifier. Instead, you can use a combination of the message's `metadata.commit_lsn` and the message's primary key(s) to uniquely identify a message. While many messages may share a `metadata.commit_lsn` (as it corresponds to the LSN of the transaction), your system should only care about capturing a single message per row per transaction.
