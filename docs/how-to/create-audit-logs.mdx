...
1. **Idempotency**: Implement idempotent processing to handle edge cases safely
 ...
 To protect against duplicates, each message includes a `idempotency_key` field that can be used for idempotent processing. This is particularly useful for discerning users who want to implement their own idempotency logic or for database sync scenarios where upserts are needed.
 ...
 {
   "metadata": {
     "idempotency_key": "base64_encoded_string_here",
     ...
   }
 }
 ```

 For regular changes (non-backfill), the idempotency key is generated by combining the transaction's position in the database log (commit_lsn) and the position within that transaction (commit_idx), then encoding it in base64. This ensures each change within a transaction has a unique identifier.

 For backfill messages, the key is generated by combining the backfill's unique identifier (backfill_id) and the primary key values of the record being processed, then encoding it in base64. This ensures each record in a backfill operation has a unique identifier, even if the same record appears multiple times.

<Note>
  Note that for backfills, this only works for tables that have primary keys, but this is already a requirement for backfill operations.
</Note>
