...
1. **Idempotency**: Implement idempotent processing to handle edge cases safely
 ...
 To protect against duplicates, you can design your processor and/or destination to be idempotent.

Messages don't have a unique identifier (coming soon - see below). Instead, you can use a combination of the message's `metadata.commit_lsn` and the message's primary key(s) to uniquely identify a message. While many messages may share a `metadata.commit_lsn` (as it corresponds to the LSN of the transaction), your system should only care about capturing a single message per row per transaction.

So, if your table's primary key is `id`, your audit table should have the columns `id` and `commit_lsn`. Then, you can create a unique index on `(id, commit_lsn)` to ensure that each row is only written once per transaction. Finally, your processor should use an `ON CONFLICT (id, commit_lsn)` clause to update the row if it already exists.

  <Info>
    `idempotency_key` is coming to messages in May 2025 ([#1251](https://github.com/sequinstream/sequin/issues/1251)).
  </Info>
