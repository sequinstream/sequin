...
1. [Sequin installed](/quickstart/self-host)
...
<Steps>
  ...
  <Step title="Leave message grouping default">
    ...
    This will ensure that messages are [grouped by primary key](/reference/ordering-and-grouping), helping eliminate race conditions as you write rows to your destination.
  </Step>
...
4. **Error handling**: Implement proper error handling and retries for your sink type:
   - For SQS, failed messages return to the queue
   - For webhooks, return non-200 status to trigger retries
   - For Kafka, manage your own offset commits
...
Here's an example of processing changes in Python. This code demonstrates several key patterns for reliable table replication:

1. Batching changes by operation type (inserts/updates vs deletes) for better performance
2. Proper type casting to handle JSON -> Postgres type conversions
3. Using `unnest()` for efficient batch operations
4. Implementing idempotent operations with `ON CONFLICT` clauses to handle duplicate messages safely

```python process_changes.py
def process_changes(changes):
    # Batch changes by operation type
    upserts = []
    deletes = []
    ...
    for change in changes:
        if change.action in ['insert', 'update', 'read']:
            ...
            record = {
                'id': uuid.UUID(change.record['id']),  # string -> uuid
                'name': change.record['name'],
                'price': Decimal(str(change.record['price'])),  # number -> decimal
                'created_at': datetime.fromisoformat(change.record['created_at'])  # string -> timestamp
            }
            upserts.append(record)
        elif change.action == 'delete':
            deletes.append(change.record['id'])

    # Batch upsert changes using unnest() for efficiency
    if upserts:
        db.execute("""
            insert into replicated_products (
                product_id, name, price, created_at
            ) 
            select * from unnest(%(records)s)
    ...
    # Batch delete changes using any() for efficiency
    if deletes:
        db.execute("""
...
With this approach, you can efficiently replicate changes from your source table to your destination table in ~40 lines of code.
...
def process_changes(changes):
    ...
    for change in changes:
        # Include source table in the record
        ...
        if change.action in ['insert', 'update', 'read']:
            ...
            deletes.append((source_table, record_id))
    ...
    if deletes:
        db.execute("""
            ...
            where (source_table, record_id) = any(%(keys)s)
        """, {'keys': deletes})
```
...
def process_changes(changes):
    ...
    for change in changes:
        ...
        if change.action in ['insert', 'update', 'read']:
            ...
            upserts.append((source_table, record))
        elif change.action == 'delete':
            deletes.append((source_table, record_id))

    if upserts:
        for source_table, record in upserts:
            # Use dynamic column name in conflict clause
            db.execute(f"""
                insert into combined_records ({source_table}_id, data)
                values (%(id)s, %(data)s)
                on conflict ({source_table}_id) do update set
                    data = excluded.data
            """, {'id': record[f"{source_table}_id"], 'data': record['data']})

    if deletes:
        for source_table, record_id in deletes:
            db.execute(f"""
                delete from combined_records 
