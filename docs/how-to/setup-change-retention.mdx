...
<Steps>
  ...
  <Step title="Configure column selection (optional)">
    Optionally, select which columns to include or exclude from the synced changes. This is useful for excluding sensitive columns (like passwords or SSNs) or including only the columns you need. You can choose to:
