...
You can use a [Change Capture Pipeline](/reference/change-capture-pipelines) to persist changes to a table in your database. Then, you can stream _that_ table to your webhook endpoint. This gives you the power to [run backfills](/reference/backfills)/replays of recent changes at any times.
...
<Steps>
  ...
  <Step title="Specify message grouping">
    ...
    While your webhook endpoint is processing a message, if a new change or version related to the same row is captured, [Sequin will hold the message back](reference/ordering-and-grouping) until your webhook endpoint acknowledges the first message. This is almost always the desired behavior.
  </Step>
