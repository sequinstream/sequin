...
You'll need a Kafka cluster and topic ready for Sequin to stream changes to. You can use either a local cluster for development or a managed Kafka service in the cloud.
...
For development with Sequin Cloud or in production environments, we recommend using a managed Kafka service such as:
...
When setting up your cloud Kafka cluster:
