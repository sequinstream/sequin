...
You can use a [Change Capture Pipeline](/reference/change-capture-pipelines) to persist changes to a table in your database. Then, you can sink _that_ table to a Sequin Stream. This gives you the power to [run backfills](/reference/backfills)/replays of recent changes at any time.
...
<Steps>
  ...
  <Step title="Specify max ack pending">
    Under "Max ack pending", specify the maximum number of messages that can be outstanding (e.g. in-flight but not yet acknowledged by a consumer) at one time.

    This setting helps prevent a consumer from getting overloaded with messages. The right value depends on your consumer's processing capacity.
  </Step>
  ...
  <Step title="Specify max waiting">
    ...
    This setting helps manage resource utilization and prevent excessive polling. The right value depends on how many parallel consumers you expect to have.
  </Step>
