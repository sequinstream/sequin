...
description: "Receive Postgres changes in AWS SQS in real-time"
---
...
Most likely, you'll want to create a FIFO queue. This will ensure your system [receives events pertaining to the same row in order](/reference/sqs#message-grouping).
...
You can use a [Change Capture Pipeline](/reference/change-capture-pipelines) to persist changes to a table in your database. Then, you can stream _that_ table to SQS. This gives you the power to [run backfills](/reference/backfills)/replays of recent changes at any times.
