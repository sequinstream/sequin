...
By the end of this guide, you'll have a working sink that automatically creates, updates, and deletes search index entries. Your users will get accurate, fresh search results as your data changes in your database.
...
- **Hard transactional guarantees:** you must update a row and query it in the same SQL transaction. Sequin provides eventual consistency, not in-transaction consistency.
...
3. A supported search engine ([Elasticsearch](/reference/sinks/elasticsearch), [Typesense](/reference/sinks/typesense), etc.)
...
<Steps titleSize="h3">
  ...
  <Step title="Configure routing">
    ...
    This will route messages to the index corresponding to the table name. You could build more complex routing that sends messages to multiple indexes based on the data in the message.
  </Step>
...
1. **After infrastructure changes**: after a search engine upgrade or migration
2. **Data model updates**: after a data model or transform function change
3. **Accidental deletion**: after a search index is accidentally deleted or dropped
