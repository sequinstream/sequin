---
title: "Testing with Sequin"
sidebarTitle: "Testing and CI"
description: "Learn how to use Sequin's CLI and API to test your sinks in CI."
---

This guide explains how best practices for unit testing your code with Sequin as well as testing Sequin in a CI environment.

## What should you test?

You should test that your sink configuration (especially the [filters](/reference/filters), [transforms](/reference/transforms), and [routing](/reference/routing)) is working as expected. You should also test the payload delivered to the destination matches your assertions - this is particularly helpful as you migrate your database schema.

In CI, you should test that Sequin can properly integrate with your infrastructure with an end-to-end test.

## Prerequisites

To properly test your Sequin configuration, you'll need to stand up test and CI environments for your database and Sequin. This is largely dependent on you've already set up these environments. Generally speaking, you'll need:

- Tooling to run Sequin in a test environment (e.g. Docker, Helm, etc.)
- A test / CI database with logical replication enabled
- A sequin.yml file that creates the sinks you need to test in your test / CI environment
...
<Steps titleSize="h3" >
  ...
  <Step title="Re-use your `sequin.yml` for testing">
    ...
    <Tip>
      Use [YAML anchors](/reference/yaml-anchors) to easily reuse elements of your sink configurations.
    </Tip>
    <Tip>
