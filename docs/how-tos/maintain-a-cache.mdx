...
First, create a stream of your source data. This stream will provide real-time updates to keep your cache fresh:
...
<Steps>
  ...
  <Step title="Create the stream">
    Click "Create Stream" to start tracking changes to your source data.
  </Step>
</Steps>

## Configure your cache

Next, set up your caching system to store the data efficiently. The configuration depends on your chosen cache technology:

### Redis configuration

For Redis, consider these key patterns and data structures:

```ruby
# Key pattern examples
"product:{id}"              # Single product cache
"products:recent"           # List of recent products
"products:category:{name}"  # Products by category

# TTL configuration
SETEX "product:123" 3600 "{json_data}"  # 1-hour expiration
```

### In-memory cache configuration

For application memory or Memcached:
...
When caching from multiple sources:

1. **Namespace your keys**: Use prefixes to avoid collisions
```ruby
"users:{id}"     # User data
...
2. **Set appropriate TTLs**: Different data types may need different expiration times
```ruby
# Critical data: shorter TTL
SETEX "orders:active:{id}" 300 "{data}"    # 5 minutes

# Reference data: longer TTL
SETEX "products:{id}" 86400 "{data}"       # 24 hours
```

3. **Consider cache hierarchies**: Layer your caches for optimal performance
```javascript
// Check local memory first, then Redis
async function getProduct(id) {
  let product = localCache.get(`product:${id}`);
  if (!product) {
    product = await redis.get(`product:${id}`);
    localCache.set(`product:${id}`, product, {ttl: 300}); // 5min local cache
  }
  return product;
}
```
...
When maintaining a cache, you need robust error handling to ensure data consistency:
...
def get_with_fallback(key)
  ...
  if value.nil?
    ...
    cache.set(key, value, ttl: 1.hour)
  end
  value
end
```
...
```python
def verify_cache_consistency(key):
    cache_value = cache.get(key)
    db_value = db.get(key)
    
    if cache_value != db_value:
        # Log inconsistency
        logger.warn(f"Cache inconsistency detected for {key}")
        # Update cache with correct value
        cache.set(key, db_value)
        # Track metric
        metrics.increment('cache.inconsistency')
```

### Recovery strategies

<Steps>
  <Step title="Circuit breaker implementation">
    Protect your system when cache becomes unavailable:
  ...
  <Step title="Fallback mechanisms">
    ...
    def get_cached_data(key)
      begin
        ...
        # Log cache failure
        logger.error("Cache connection failed: #{e.message}")
      end
    
      # Fallback to database
      db_value = database.fetch(key)
...
Implement the cache-aside pattern for better control:
...
async function getCachedData(key) {
  ...
  if (!data) {
    ...
    await cache.set(key, data, 'EX', 3600); // 1 hour TTL
  }
...
Maintain consistency with write-through caching:
...
def update_data(key, value):
    ...
    related_keys = find_related_keys(key)
    cache.delete_many(related_keys)
```
...
def warm_cache_for_user(user_id)
  ...
  common_queries.each do |query, limit|
    data = database.fetch_user_data(user_id, query, limit)
    cache.set("user:#{user_id}:#{query}", data)
  end
end
```
...
async function backgroundRefresh() {
  ...
  for (const key of keys) {
    ...
    // Refresh keys nearing expiration
    if (ttl < 300) { // Less than 5 minutes
      const newData = await fetchFreshData(key);
...
When deploying your cache maintenance pipeline to production, consider these key aspects:

### Cache infrastructure setup

<Steps>
  <Step title="Scale your cache">
    Ensure your cache infrastructure can handle production load:
    
    - Configure appropriate memory allocation
    - Set up cache clustering if needed
    - Enable persistence for Redis/similar caches
  </Step>
  ...
  <Step title="Set up Sequin">
    Configure your Sequin pipeline:
    ...
    ```yaml
    # sequin.production.yaml
    cache:
      redis:
        url: ${REDIS_PRODUCTION_URL}
        maxMemory: "2gb"
  ...
  <Step title="Deploy configurations">
    Apply your production settings:

    ```bash
    sequin config apply sequin.production.yaml --context=production
    ```
  </Step>
</Steps>
...
1. **Graceful degradation**: Implement fallbacks if cache becomes unavailable
2. **Circuit breakers**: Prevent cascade failures if cache is overloaded
3. **Cache warming**: Schedule periodic cache warming to maintain performance
4. **Monitoring alerts**: Set up alerts for cache health metrics

## Monitoring and maintenance

Keep your cache healthy with proper monitoring and maintenance:

### Key metrics to track

- **Hit rate**: Aim for >80% cache hits
- **Memory usage**: Monitor for approaching limits
- **Eviction rate**: Watch for unexpected spikes
- **Latency**: Track read/write times
- **Stale entries**: Monitor TTL expiration patterns

### Common maintenance tasks

1. **Regular health checks**:
   ```bash
   redis-cli --stat  # For Redis monitoring
   ```
...
3. **Performance tuning**:
   ```bash
   redis-cli info | grep used_memory  # Memory usage
   ```
...
4. **Performance optimization**: Improving cache patterns

To re-warm your cache:

<Steps>
  <Step title="Prepare for re-warming">
    1. Identify affected cache keys
    2. Estimate data volume
    3. Plan for increased database load
  </Step>

  <Step title="Execute re-warming">
    Use Sequin's rewind feature to replay data:
    
    1. Go to your Consumer Group/Webhook Subscription
    2. Click "Rewind"
    3. Choose the appropriate time range
  </Step>

  <Step title="Monitor progress">
    Track re-warming progress:
...
<Info>
  Consider implementing a gradual re-warming strategy for large datasets to minimize system impact.
</Info>
