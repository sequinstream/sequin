...
3. Either **[Create a Consumer Group](#create-a-consumer-group)** or **[Create a Webhook Subscription](#consume-with-webhooks)**: Use either tool to process the rows
...
First, create a stream of your source table:
...
<Steps>
  ...
  <Step title="Select your source table">
    Under "Source configuration", select the table you want to replicate.

    You can optionally add filters to narrow down which rows you want to replicate. For example, maybe you only want to replicate `users` that have made a purchase:

    <Frame>
      <img style={{maxWidth: '500px'}} src="/images/how-tos/replicating-tables/source-config.png" alt="Source configuration" />
    </Frame>
  </Step>
...
We recommend adding a column to your destination table that corresponds to the primary key of your source table. For example, if your source table has a `user_id` primary key:

```sql
create table replicated_users (
  user_id uuid primary key,  -- matches source table PK
  name text,
  email text,
...
<Steps>
  ...
  <Step title="Configure the source">
    ...
    For "Where should the Consumer Group start?", leave the default option selected, "At the beginning of the table." This means your Consumer Group will process all existing rows, as well as any new ones.
  </Step>
