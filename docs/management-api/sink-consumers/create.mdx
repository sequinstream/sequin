...
<ParamField body="annotations" type="object">
  Additional metadata attached to this sink consumer. Annotations can be any JSON object (not a list or a scalar).
  This data can be referenced in your [transform](/reference/transforms), to have one function include sink-specific data if that function is attached to multiple sinks.
</ParamField>
