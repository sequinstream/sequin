...
description: 'Stream Postgres changes to an Azure Event Hubs topic'
---
...
- Configure an Event Hubs topic to receive database changes
 ...
 <Step title="Create an Event Hub Sink" icon="microsoft">
  ...
  <Steps>
    ...
    <Step title='Configure "Event Hub"'>
      ...
      <Frame>
        <img style={{ maxWidth: '700px' }} src="/images/quickstart/azure-eventhubs/eventhub-config-card.png" alt="Event Hub configuration card" />
      </Frame>
    </Step>
  ...
  <Step title="See changes flow to your Event Hub" icon="waveform-lines">
    ...
    <Step title="Messages tab">
      ...
      <Frame>
        <img style={{ maxWidth: '700px' }} src="/images/quickstart/azure-eventhubs/messages-tab.png" alt="List of recently delivered messages" />
      </Frame>
      ...
      <Step title="View in Azure Portal">
        ...
        You should see a list of events. The events each contain a body that is base64 encoded JSON.

        To view the contents of a message, copy one of the event bodies and decode it with `base64 --decode` in your terminal:

        ```bash
        echo "eyJhdHRyaWJ1dGVzIjp7InRyYWNlX2lkIjoiZWE..." | base64 --decode
        ```

        The messages are [`read` events](/reference/messages#change-messages) from the initial backfill of the `products` table.
...
<CardGroup cols={2}>
  ...
  <Card title="Guide: Setting up an Event Hub sink" icon="microsoft" href="/how-to/stream-postgres-to-azure-eventhubs">
