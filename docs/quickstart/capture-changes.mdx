...
<Steps titleSize="h2">
  <Step title="Create a WAL Pipeline" icon="list">
    ...
    2. Under "Source configuration", the `public.groceries` table should be selected already:
    ...
    8. Give your WAL Pipeline a name (i.e. `groceries_to_sequin_events`) and click **Create WAL Pipeline**.

    <Check>
    Sequin will now capture every change to the `groceries` table and write it to your event log.
    </Check>
  </Step>
  ...
  <Step title="Make some changes" icon="play">
    ...
    3. Insert a new produce item:

        ```bash
        INSERT INTO public.groceries (name, section, checked) VALUES ('Kale', 'produce', false);
        ```
    ...
    3. Query your event log table to see the insert:
        ...
        action    | insert
        record_pk | 9
        record    | {"id": 9, "name": "Kale", "section": "produce", "checked": false, "updated_at": "2024-10-28T21:37:53", "inserted_at": "2024-10-28T21:37:53"}
        changes   |
        ```

    4. Mark the Kale as checked:
    ...
    5. Query your event log again:
        ...
        action    | update
        record_pk | 9
        record    | {"id": 9, "name": "Kale", "section": "produce", "checked": true, "updated_at": "2024-10-28T21:39:21", "inserted_at": "2024-10-28T21:37:53"}
        changes   | {"checked": "true", "updated_at": "2024-10-28T21:37:53"}
        ```

    6. Finally, delete the region:

        ```bash
        DELETE FROM public.groceries WHERE name = 'Kale';
        ```
  ...
  <Step title="Stream your events" icon="arrow-right">
    Now that you're capturing changes, you can create a Sequence for your event log table and stream the events to consumers:
    ...
    You can now use the [Consume API](/quickstart/consume-with-api) or [webhooks](/quickstart/consume-with-webhooks) to stream these events to your applications.
  </Step>
</Steps>
