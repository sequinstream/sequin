...
description: 'Consume rows from your Sequence using the Consume API'
---

With Sequin's **Consume API**, you can pull rows from your Sequences using HTTP endpoints. You'll create a **Consume API Endpoint** that can have multiple consumers (processes or workers) pulling rows in parallel. Each consumer acknowledges rows after processing them, and if a consumer fails, the rows will be made available to other consumers.
...
<Steps titleSize="h2">
  <Step title="Create a Consume API Endpoint" icon="users">
    After connecting a database and creating a Sequence, you can consume rows from your Sequence using a Consume API Endpoint:
    ...
    5. Give your Consume API Endpoint a name (i.e. `produce_groceries`) and click **Create Consume API Endpoint**.
  ...
  <Step title="Receive messages" icon="arrow-down-to-line">
    ...
    1. Copy the "Receive messages" curl command from the **Overview** tab. It looks like this:
        ...
        ```bash
        curl -X GET 'https://api.sequinstream.com/api/http_pull_consumers/produce_groceries/receive?batch_size=10' \
    ...
    5. Now, acknowledge the message using the `/ack` endpoint. Copy the "Acknowledge messages" command from your dashboard and replace the `<ack_id>` placeholder with the `ack_id` from the message you received:
        ...
        ```bash
        curl -X POST 'https://api.sequinstream.com/api/http_pull_consumers/produce_groceries/ack' \
          -H "Authorization: Bearer YOUR_API_TOKEN" \
  ...
  <Step title="Insert and update rows" icon="play">
    To learn how Sequin works, run a couple of `insert` and `update` operations on the `public.groceries` table:
    ...
    2. Add a new produce item to your grocery list:
        ...
        ```bash
        INSERT INTO public.groceries (name, section, checked) VALUES ('Grapes', 'produce', false);
        ```
    ...
    4. Mark the Grapes as checked:

        ```bash
        UPDATE public.groceries SET checked = true WHERE name = 'Grapes';
        ```
...
<CardGroup cols={1}>
  <Card title="Quickstart: Consume rows via webhooks" icon="gauge-max" href="/quickstart/consume-with-webhooks">
    See how to subscribe to your Sequence and receive rows as they change.
  </Card>
