...
<Steps titleSize="h2">
 ...
 <Step title="Create an Elasticsearch sink" icon="plug">
  ...
  <Steps>
    ...
    <Step title="Add a transform">
      Open the **Transform** card, click **+ Create new transform** and use the following Elixir function:
      ...
      Name the transform `products-elasticsearch`, save it, and select it in the list.
    </Step>

    <Step title="Configure a backfill">
      Open **Initial backfill** and choose **Backfill all rows** so the existing data is loaded.
    </Step>
 ...
 <Step title="Query your data in Elasticsearch" icon="waveform-lines">
   Once the backfill completes you can query the index:
 ...
 <Step title="See changes flow to Elasticsearch" icon="waveform-lines">
   In the sink's overview page the **Health** indicator should turn green once data flows.
