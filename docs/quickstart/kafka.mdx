...
<Steps titleSize="h2">
 <Step title="Boot Kafka (optional)" icon="database">
  If you don't already have Kafka running, start Kafka with Docker Compose. You can use the following commands to get started:

  ```bash
  mkdir kafka-quickstart && cd kafka-quickstart
  wget https://raw.githubusercontent.com/sequinstream/sequin/main/docs/static/kafka-docker-compose.yaml
  docker compose up
  ```

  <Accordion title="Kafka docker-compose.yaml">
    ```yaml
