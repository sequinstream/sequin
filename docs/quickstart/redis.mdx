...
- Set up a local development environment with Sequin
...
- A Redis instance (version 5.0 or later for stream support)
- Connection credentials for your Redis instance

<Steps titleSize="h2">
 <QuickstartInitialSteps />
 <Step title="Create a Redis Sink" icon="database">
  ...
  <Steps>
    ...
    <Step title='Configure "Redis Configuration"'>
      ...
      For the "Host" field, if connecting to Redis locally, use `host.docker.internal` instead of `localhost`. This is because you're running Sequin in Docker.

      Complete the rest of the fields:

      - Port: The port Redis is listening on (default: 6379)
      - Username: (Optional) Your Redis username
      - Password: (Optional) Your Redis password
      - Stream Key: The key to use for the Redis stream (e.g., `products-stream`)
      - Database: The Redis database number (default: 0)
      - TLS: Toggle on if your Redis instance requires TLS

      <Frame>
        <img style={{ maxWidth: '700px' }} src="/images/quickstart/redis/redis-config-card.png" alt="Redis configuration card" />
      </Frame>
    </Step>
  ...
  <Step title="See changes flow to your Redis stream" icon="waveform-lines">
    ...
    <Step title="Messages tab">
      ...
      <Frame>
        <img style={{ maxWidth: '700px' }} src="/images/quickstart/redis/messages-tab.png" alt="List of recently delivered messages" />
      </Frame>
...
<CardGroup cols={2}>
  ...
  <Card title="Guide: Setting up a Redis sink" icon="database" href="/how-to/stream-postgres-to-redis">
