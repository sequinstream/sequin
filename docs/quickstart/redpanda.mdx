...
description: 'Stream Postgres changes to a Redpanda cluster'
---
  ...
  <Step title="See changes flow to your Redpanda topic" icon="waveform-lines">
    ...
    <Step title="Messages tab">
      ...
      <Step title="View in Redpanda Console">
        In the Redpanda Console, in the "Topics" tab of your cluster, navigate to the `products` topic. You should see the messages that were sent from Sequin. These are [`read` events](/reference/messages#change-messages) from the initial backfill of the `products` table:
