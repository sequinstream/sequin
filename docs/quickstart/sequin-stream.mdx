...
<Steps titleSize="h2">
 ...
 <Step title="Create a Sequin Stream Sink" icon="stream">
  ...
  <Step title="See changes flow to your Sequin Stream" icon="waveform-lines">
    ...
    <Step title="Messages tab">
      ...
      <Step title="Make some changes">
        ...
        ```bash
        curl -X GET "http://localhost:7376/api/http_pull_consumers/products-stream/receive?batch_size=10" \
        ...
        ```bash
        curl -X POST "https://api.sequinstream.com/api/http_pull_consumers/products-stream/ack" \
          -H "Authorization: Bearer YOUR_API_TOKEN" \
