...
<Steps titleSize="h2">
  ...
  <Step title="View the playground database" icon="database">
    To help you try Sequin quickly, Sequin's Docker Compose file creates a logical database called `sequin_playground` with a sample dataset in the `public.groceries` table:

    ```bash public.groceries
    id |  name  | section | checked |        inserted_at         |         updated_at
----+--------+---------+---------+----------------------------+----------------------------
  1 | Apple  | produce | f       | 2024-11-08 23:35:30.021632 | 2024-11-08 23:35:30.021632
  2 | Banana | produce | f       | 2024-11-08 23:35:30.021632 | 2024-11-08 23:35:30.021632
  3 | Bread  | bakery  | f       | 2024-11-08 23:35:30.021632 | 2024-11-08 23:35:30.021632
  4 | Milk   | dairy   | f       | 2024-11-08 23:35:30.021632 | 2024-11-08 23:35:30.021632
(4 rows)
    ```


    The Sequin server automatically connects to this database. Go take a look:
  ...
  <Step title="View the groceries stream" icon="list">
    Now, view the pre-configured `groceries` stream:

    1. In the Sequin web console, click **Streams** in the sidebar.

    2. The `public.groceries` table should already have a [Stream](/how-sequin-works#streams) configured:

        <Frame>
          <img src="/images/quickstart/grocerie-stream.png" alt="Regions stream" />
        </Frame>
  </Step>
</Steps>
...
You are running Sequin, connected to a playground database, and have a Stream setup for the `groceries` table. Now for the fun part – streaming! You can consume rows from the stream using a Webhook Subscription or the Consumer Group API:
