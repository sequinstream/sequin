...
<Steps titleSize="h2">
  ...
  <Step title="Connect the playground database" icon="database">
    To help you try Sequin quickly, Sequin's Docker Compose file creates a logical database called `sequin_playground` with a sample dataset in the `public.groceries` table. Let's connect to it:
  ...
  <Step title="Create a Sequence" icon="list">
    Now, create a [Sequence](/how-sequin-works#sequences) to stream changes from the `public.groceries` table:

    1. Navigate to the **Sequences** tab and click the **Create Sequence** button.

    2. The `public.groceries` table should already be selected by default.

    3. Select the column Sequin should use to order changes. In this case select the `updated_at` column:

        <Frame>
          <img src="/images/quickstart/create-sequence.png" alt="Create Sequence" />
        </Frame>
...
You are running Sequin, connected to a playground database, and have a Sequence setup for the `groceries` table. Now for the fun part – streaming! You can stream rows via a Webhook Subscription or the Consume API:
