 ...
 <Step title="Create a Typesense Sink" icon="plug">
  ...
  <Steps>
    ...
    <Step title="Setup a transform">
      In the [Transform](/reference/transforms) card, click the toggle to view existing transforms. None exist, so click the "+" icon which will open a window for creating a new transform.

      For "Transform name", put `products-typesense`. For "Transform type", select "Function transform".

      In the "Function transform" field, input the following code:
      ...
      <Frame>
       ...
       You can close the transform window, which should bring you back to the Typesense sink create page.
    ...
    <Step title='Configure Typesense'>
      ...
      <Frame>
        <img  src="/images/quickstart/typesense/config-card.png" alt="Typesense configuration card" />
      </Frame>
    </Step>
