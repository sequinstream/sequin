...
description: 'Reference for Sequin transaction annotations'
---

Transaction annotations allow you to attach metadata to database changes. When you set annotations, all change messages following the annotation will include the annotations [in their metadata](/reference/messages#change-messages).
...
- Annotations are only supported for [change messages](/reference/messages#change-messages). They are not supported for [row messages](/reference/messages#row-messages).

## Message format

Annotations appear in the `metadata.transaction_annotations` field of [change messages](/reference/messages#change-messages):
