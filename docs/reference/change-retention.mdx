...
description: "Retain change messages in a Postgres table"
---

## About

In Sequin, [change messages](/reference/messages#change-messages) are ephemeral. Once they're sent to a sink, they're deleted.

For some sinks and use cases, it can be desirable to retain change messages for some period of time. Then, you can take advantage of Sequin's [backfill](/reference/backfills) feature to replay the changes to the sink.
...
Below are some examples of messages that Change Capture Pipelines capture:
