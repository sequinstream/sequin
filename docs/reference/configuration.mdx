...
- `SEQUIN_METRICS_USERNAME`: Username required to access the metrics endpoint (optional)
- `SEQUIN_METRICS_PASSWORD`: Password required to access the metrics endpoint (optional)

  If **either** variables are set, Sequin secures the `/metrics` endpoint with HTTP Basic Authentication. When unset, the endpoint is publicly accessible (recommended only for local development).
