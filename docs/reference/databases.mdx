...
Sequin works with any PostgreSQL database version 12 or higher that supports the [Logical Replication](https://www.postgresql.org/docs/current/logical-replication.html).

You can find guides for connecting to local and hosted databases in the [connect postgres](/connect-postgres) guide.

## Database connection credentials

Sequin requires the following credentials to connect to your database:

- **Host**: The hostname of your database
- **Port**: The port number of your database
- **Database**: The name of your database
- **Username**: The username of a database user with the [necessary permissions](/reference/databases#database-user-permissions)
...
Use the [Sequin CLI](/reference/cli/overview) to create a secure tunnel between Sequin Cloud and a local database.
...
When connecting Sequin to your database, it's recommended to create a dedicated database user with the following permissions:
...
You can create a user with the following commands:
...
Please create a strong password for your database user. You can generate a secure password using the following command:
...
If the output isn't `logical`, you'll need to enable it. The process for enabling logical replication varies by provider. You can find more information in the following guides:

- [Self-hosted PostgreSQL](/connect-postgres): Modify `postgresql.conf` to set `wal_level = logical`
...
By default, Sequin looks for a publication named `sequin_publication`.
...
By default, Sequin looks for a slot named `sequin_slot`.
...
**Use meaningful names**: Name slots clearly to identify their purpose, e.g., `sequin_prod_slot`, `sequin_audit_slot`
