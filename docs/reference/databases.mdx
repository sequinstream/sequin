...
Sequin relies on the `pg_logical_emit_message` function for heartbeat messages and backfills. This function is included automatically in Postgres 14 and later.
...
If you require backfills, we recommend upgrading to Postgres 14 or later. If this is not an option, please [contact support](mailto:support@sequinstream.com) to discuss alternative approaches.
