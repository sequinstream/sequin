...
sinks:
  - name: "my-sink"                # Required, unique name for this sink
    ...
    table: "public.users"          # Optional, [schema.]table to stream
    schema: "public"               # Optional, schema to stream
    ...
    actions:                       # Optional, defaults to all
      ...
      - update
      - delete
    filters:                       # Optional, filter rows in the stream
      - column_name: "status"
        operator: "="
...
A sink must specify either a `table` or a `schema`.

* If you specify a `table`, Sequin will stream all changes to that table.
* If you specify a `schema`, Sequin will stream all changes to all tables in that schema.
