...
Configure Sequin resources like databases, sinks, and transforms using YAML.

You can provide YAML configuration to Sequin in two ways:

1. At runtime using [Sequin CLI](/reference/cli/overview) using the `sequin config export|plan|apply` command group
1. At start up using `CONFIG_FILE_PATH` or `CONFIG_FILE_YAML` [environment variables](/reference/configuration#configuration-file)

### Names

In general, resources in Sequin are uniquely identified both by `id` and by `name`. For `sequin.yaml`, the `name` is used to reference resources in the YAML file.


For instance, when you create a `transform`, you specify a `name` that must be unique across all transforms in your Sequin account. Then, when you create a `sink`, you can reference the `transform` by `name`.
...
When you apply a `sequin.yaml` file, Sequin performs upserts for each resource in the file. The `name` is used to match resources in the file to resources in the database. So:

* If you change the `name` of a resource in the Sequin console, you must also update the `name` in the `sequin.yaml` file to match.
* If you change the `name` of a resource in the `sequin.yaml` file, `sequin config apply` will create a new resource with the new name.

### Deletes

Currently, `sequin.yaml` does not support deleting resources. If you remove a resource from the `sequin.yaml` file, Sequin will not delete the resource from the database.

Both the Sequin console and the [management API](/reference/management-api) support deletes.
...
Creating accounts via YAML is only supported on self-hosted Sequin. The feature is intended for use in development or CI/CD workflows, where you may need to create accounts in a non-interactive way.
...
Creating API tokens via YAML is only supported on self-hosted Sequin. The feature is intended for use in development or CI/CD workflows, where you may need to create API tokens in a non-interactive way.
...
databases:
  - name: "my-database"             # Required, unique identifier
    ...
    await_database:                 # Optional, see "database connection retry" below
...
* The `database` field references a [database] by name. See [databases](#database-configuration) for more information.
* The `table` field references a table in the database by `[schema.]table`. `schema` is optional and defaults to `public`.
* The `transform` field references a [transform] by name. See [sink transforms](#sink-transforms) below for more information.
* Filters are optional. See [sink filters](#sink-filters) below for more information.
...
Auth type and auth value jointly determine the authentication method used to connect to the Elasticsearch cluster.

* `api_key`: Use an API key with the `auth_value` as the key.
* `basic`: Use basic authentication with the `auth_value` as `username:password`.
* `bearer`: Use a Bearer token with the `auth_value` as the token. Do **not** include the `Bearer ` prefix in the `auth_value`.
