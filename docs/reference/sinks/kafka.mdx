...
description: "Reference for the Kafka sink"
---
...
Kafka maintains message order within partitions. Messages sent to the same partition are guaranteed to be delivered in the order they were sent.

### Partitioning Strategy

Sequin uses the hash of the record's primary key(s) as the partition key. This ensures that messages related to the same record are sent to the same partition, maintaining ordering for updates to the same record.
