...
Sequin provides exactly-once delivery under most circumstances, but [exactly-once delivery is not always possible](https://blog.sequinstream.com/at-most-once-at-least-once-and-exactly-once-delivery/). Therefore, Sequin implements at-least-once delivery with [idempotency keys](/references/messages#idempotency). 
...
When sink consumers can't keep up with incoming messages from Postgres, Sequin will still advance the Postgres replication slot and buffer messages in memory. When the in-memory buffer fills up, the `load_shedding_policy` will be triggered. There are two available load shedding policies:

`pause_on_full` (default) — pauses replication for all consumers on the slot until the buffer has room again. In this case, Postgres guarantees no lost messages. However, in order to do this, Postgres has to save the messages on disk. This means the replication slot can grow without bound. Unmanaged, this will cause your database to run out of disk space.

`discard_on_full` — Sequin will continue reading messages from the replication slot. However, messages intended for the overloaded sink consumer(s) will be dropped. Other sink consumers will not be impacted. This limits possibility that your database will run out of disk space but will cause consumers to be inconsistent with the state of the database.
