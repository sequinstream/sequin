---
title: "Redis sink"
sidebarTitle: "Redis sink"
description: "Reference for the Redis sink"
---

The Redis sink sends messages to a Redis stream using the XADD command.
...
Sequin sends messages to Redis streams using the XADD command. Each message contains the following fields:

For record messages:
- `record`: The JSON-encoded record data
- `metadata`: JSON-encoded metadata about the record

For change messages:
- `record`: The JSON-encoded record data
- `changes`: JSON-encoded changes to the record
- `action`: The type of change (e.g., "insert", "update", "delete")
- `metadata`: JSON-encoded metadata about the event
...
<Info>
  If you want to see logging/alerting for message size rejections, please contact support.
</Info>

## Message ordering

Redis streams maintain message order within a single stream. Messages are assigned auto-incrementing IDs by Redis when they are added to the stream, ensuring strict ordering of messages.
