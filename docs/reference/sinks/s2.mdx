...
S2 currently supports records up to **4 MiB** (the maximum size of a single append request). If a message exceeds this limit S2 returns `payload too large` and Sequin will keep retrying. The typical fix is to:
...
- Record size > 4 MiB (**413 Payload Too Large**).

For live verification you can tail the stream with the S2 CLI:

```bash
s2 read s2://<basin>/<stream>
```
