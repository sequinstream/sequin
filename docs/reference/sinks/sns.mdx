...
You can run SNS locally using [LocalStack](https://localstack.cloud/).

```bash
docker run -d -p 4566:4566 -e SERVICES=sns localstack/localstack
```

Then, include the `use_emulator: true` and `emulator_base_url` configuration to send messages to the local SNS topic:
