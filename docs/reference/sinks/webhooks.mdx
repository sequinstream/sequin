...
- **When "Batch messages" is disabled (default):**
    The request body will contain a single message object. The format depends on whether the sink is configured for change messages or row messages (see below). You can also use a [transform](/reference/transforms) to customize the shape of the message.
...
The format of the individual *message objects* depends on whether you've configured the sink for change messages or row messages:
