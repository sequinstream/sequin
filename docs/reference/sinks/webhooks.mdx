...
description: "Reference for the Webhook sink"
---
...
Your webhook endpoint must return a `2XX OK` response after successfully processing a batch of messages. Sequin will consider any response code outside the range of `200` to `299` a failure, and will [mark the message for re-delivery](#retry-behavior).

<Note>
  All messages in a batch are either acknowledged together or fail together. There is no way to send Sequin a response that partially acknowledges a batch.
</Note>
...
When you specify a "Batch size" greater than `1`, Sequin may include more than one message for the same group in a single request. For clarity, Sequin will never include more than the specified "Batch size" messages in a single request, but may include fewer.
