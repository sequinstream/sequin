...
description: Reference for Sequin's SQL enrichment functionality. Use SQL enrichment to add additional data to your messages before they are sent to sinks.
---

SQL enrichment allows you to enrich your messages with additional data from your database before they are sent to the sink destination. This is useful for:
...
When you configure SQL enrichment on a sink consumer, Sequin will:

1. Batch messages that need to be enriched
2. Execute your SQL enrichment query with the primary keys of those messages
...
## SQL enrichment syntax

SQL enrichment functions must:
...
Your SQL enrichment query must select all primary key columns from the source table. These are used to match the enrichment results back to the original messages.
...
SQL enrichment functions must use the `$1` syntax for parameterization. This allows Sequin to safely batch multiple messages into a single query.
