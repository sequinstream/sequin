...
<Steps titleSize="h3">
  ...
  <Step title="Create three webhook subscriptions to fan out messages" icon="signal-stream">
       Create three [webhook subscriptions](/how-sequin-works#webhooks) (one for inventory, one for shipping, and one for invoicing) that capture changes from the `orders` table and delivers them to the relevant service:
