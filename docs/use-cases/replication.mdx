...
<Steps titleSize="h3">
  ...
  <Step title="Create a Sequence" icon="list-ol">
    Add a Sequence to your `products` table to stream rows to your search index:
  ...
  <Step title="Create a Webhook Subscription" icon="webhook">
    Create a [webhook subscription](/how-sequin-works#webhooks) to deliver rows from your `products` sequence to your search index (which you'll handle in the next step).

    1. Navigate to the **Webhook Subscriptions** tab and click **Create Subscription**.
    2. Select the `products` sequence you just created.
    ...
    <Check>
      You've set up a webhook subscription to deliver rows from your `products` sequence to your application's endpoint.
    </Check>
