...
This guide shows you how to implement side effects with Sequin.
...
<Steps titleSize="h3">
  ...
  <Step title="Create a consumer to capture `inserts` on the `users` table" icon="signal-stream">
       Create a [webhook subscription](/how-sequin-works#webhooks) that captures changes from the `users` table. It'll then send an HTTP request to your application with the new user's information (which you'll handle in the next step).
