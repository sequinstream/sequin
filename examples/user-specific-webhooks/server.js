...
const SEQUIN_API_TOKEN =
  "MMCNQkmMiUh3qCyPMief585KXHBL-X4jZEvWyqnnOyaVyHBssKKNuo6xUHhbfZu3"; // (e.g., the token you copied from the console)
...
app.post("/subscribe", async (req, res) => {
  ...
  try {
    ...
    const endpointResp = await axios.post(
    ...
    const sinkResp = await axios.post(
      `${SEQUIN_API_BASE}/sinks`,
      {
        name: sinkName,
        status: "active",
        database: "local", // source database name
        table: "notifications", // source table to watch
        filters: [
