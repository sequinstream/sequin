defmodule Sequin.Consumers.PathTransform do
  ...
  defp validate_path(changeset) do
    ...
    |> validate_format(
      :path,
      ~r/^(record|changes|action|metadata)$|^(record|changes|metadata)\.([a-zA-Z0-9_]+)$|^metadata\.(table_schema|table_name|commit_timestamp|commit_lsn|transaction_annotations|sink)$|^metadata\.transaction_annotations\.[a-zA-Z0-9_]+$|^metadata\.sink\.(id|name)$/,
