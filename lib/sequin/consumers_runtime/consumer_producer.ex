defmodule Sequin.ConsumersRuntime.ConsumerProducer do
  ...
  defp handle_receive_messages(%{demand: demand} = state) when demand > 0 do
    ...
    {:ok, delievered_seqs} = ConsumerIdempotency.delivered_messages(state.consumer.id, messages)
    messages = Enum.reject(messages, &(&1.seq in delievered_seqs))

    unless delievered_seqs == [] do
      Logger.warning(
        "Received #{length(delievered_seqs)} messages for consumer #{state.consumer.id} that have already been delivered",
        seqs: delievered_seqs
      )
    end
  ...
  def ack({consumer, test_pid}, successful, failed) do
    ...
    :ok = ConsumerIdempotency.mark_messages_delivered(consumer.id, successful_messages)
