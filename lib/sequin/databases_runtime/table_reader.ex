defmodule Sequin.DatabasesRuntime.TableReader do
  ...
  # @spec fetch_batch_rows(
  #         db_or_conn :: Postgres.db_conn(),
  #         table :: PostgresDatabaseTable.t(),
  #         min_cursor :: KeysetCursor.cursor(),
  #         opts :: Keyword.t()
  #       ) :: {:ok, %{rows: list(), next_cursor: KeysetCursor.cursor() | nil}} | {:error, any()}
  # def fetch_batch_rows(db_or_conn, %PostgresDatabaseTable{} = table, min_cursor, opts \\ []) do
  #   timeout = Keyword.get(opts, :timeout, :timer.minutes(1))
  #   {sql, params} = fetch_batch_query(table, min_cursor, opts)
