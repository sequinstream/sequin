defmodule Sequin.Runtime.HttpPushPipeline do
  ...
  def handle_batch(:default, messages, batch_info, context) do
    ...
    if Map.get(consumer.sink, :via) do
      case push_to_sqs(consumer, http_endpoint, message_data, req_opts, context) do
        :ok ->
          {:ok, messages, context}
  ...
  defp push_to_sqs(consumer, http_endpoint, message_data, req_opts, context) do
    %{sqs_client: sqs_client} = context

    # Build the HTTP request details
    headers = http_endpoint.headers
    encrypted_headers = http_endpoint.encrypted_headers || %{}
    headers = Map.merge(headers, encrypted_headers)

    # The method can be overridden in req_opts
    ...
    envelope = %{
      ...
      metadata: %{
        ...
        max_retry_count: consumer.max_deliver,
        # 30 seconds, hardcoded as specified
