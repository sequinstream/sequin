defmodule Sequin.Runtime.SlotMessageHandler do
  ...
  * `:partition_count` - The total number of partitions
  ...
  def flush_high_watermark_wal_cursor(%Context{} = ctx, high_watermark_wal_cursor) do
    %{replication_slot_id: slot_id} = ctx

    name = via_tuple(slot_id, 0)
    GenServer.cast(name, {:flush_high_watermark_wal_cursor, high_watermark_wal_cursor})
  end
  ...
  @impl GenServer
  def handle_cast({:flush_high_watermark_wal_cursor, high_watermark_wal_cursor}, %State{} = state) do
    Enum.each(state.consumers, fn consumer ->
      :ok = SlotMessageStore.put_high_watermark_wal_cursor(consumer, high_watermark_wal_cursor)
    end)
