defmodule Sequin.Sinks.Nats.ConnectionCache do
  ...
  Cached connections are invalidated and recreated when their NATS sink's
    ...
    def lookup(cache, sink) do
      ...
      cond do
        ...
        is_pid(entry.conn) and !Process.alive?(entry.conn) ->
          Logger.warning("Cached NATS connection was dead upon lookup", sink_id: sink.connection_id)
          {:error, :not_found}

        entry.options_hash != new_hash ->
          Logger.info("Cached NATS sink connection was stale", sink_id: sink.connection_id)
          {:error, :stale}
  ...
  defmodule State do
    ...
    def new(opts) do
      ...
      stop_fn = Keyword.get(opts, :stop_fn, &Gnat.stop/1)
