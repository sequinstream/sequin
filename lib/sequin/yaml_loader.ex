defmodule Sequin.YamlLoader do
  ...
  defp upsert_database(account_id, %{"name" => name} = database_attrs, opts) when is_binary(name) do
    ...
    |> case do
      {:ok, database} ->
        ...
        with {:ok, database_attrs} <- parse_database_update_attrs(database_attrs, database.replication_slot) do
  ...
  defp parse_database_update_attrs(database_attrs, replication_slot) do
    ...
    cond do
      not is_nil(slot_name) and slot_name != replication_slot.slot_name ->
        {:error,
         Error.validation(
           summary:
      ...
      not is_nil(pub_name) and pub_name != replication_slot.publication_name ->
        {:error,
         Error.validation(
           summary:
  ...
  defp parse_database_attrs(database_attrs) do
    cond do
      ...
      is_nil(database_attrs["slot"]) and is_nil(database_attrs["slot_name"]) ->
        {:error, Error.validation(summary: "Specify either slot or slot_name, not both")}
      ...
      is_nil(database_attrs["publication"]) and is_nil(database_attrs["publication_name"]) ->
        {:error, Error.validation(summary: "Specify either publication or publication_name, not both")}
