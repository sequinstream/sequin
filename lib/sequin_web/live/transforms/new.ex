defmodule SequinWeb.TransformsLive.New do
    ...
    case PathTransform.changeset(%PathTransform{}, params) do
      %Ecto.Changeset{valid?: true} ->
        {:noreply,
         ...
         |> push_navigate(to: ~p"/sequences/#{socket.assigns.sequence.id}")}
  ...
  defp encode_test_messages(test_messages, path) do
    ...
    |> Enum.map(fn message ->
      %{
        original: Message.to_external(original_consumer, message),
        transformed: Message.to_external(consumer, message)
      }
    end)
    |> dbg()
  end
