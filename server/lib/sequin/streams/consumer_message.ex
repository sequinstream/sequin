defmodule Sequin.Streams.ConsumerMessage do
  ...
  typed_schema "consumer_messages" do
    ...
    field :message_id, Ecto.UUID, primary_key: true
  ...
  def changeset(consumer_message, attrs) do
    ...
    |> validate_required([:consumer_id, :message_seq, :message_key, :message_id, :state, :deliver_count])
  end
  ...
  def where_message_id(query \\ base_query(), message_id) do
    from([consumer_message: cm] in query, where: cm.message_id == ^message_id)
  end
  ...
  def join_message(query \\ base_query(), stream_id) do
    from [consumer_message: cm] in query,
      ...
      on: m.id == cm.message_id and m.stream_id == ^stream_id,
      as: :message
  end
