defmodule Sequin.Sources.BackfillPostgresTableWorkerTest do
  ...
  describe "perform/1" do
    ...
    test "backfills messages and creates next job when rows are returned", %{postgres_replication: postgres_replication} do
      ...
      assert Enum.at(messages, 0).subject == "#{postgres_replication.postgres_database.slug}.public.#{@table_name}.1"
      assert Enum.at(messages, 1).subject == "#{postgres_replication.postgres_database.slug}.public.#{@table_name}.2"
