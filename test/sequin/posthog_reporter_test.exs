defmodule Sequin.Telemetry.PosthogReporterTest do
  ...
  test "buffers and flushes events in batches" do
    pid = start_supervised!({PosthogReporter, [publish_interval: 3, name: Module.concat(__MODULE__, PosthogReporter)]})
  ...
  test "flushes remaining events on termination" do
    pid =
      start_supervised!(
        {PosthogReporter, [publish_interval: :timer.minutes(1), name: Module.concat(__MODULE__, PosthogReporter)]}
      )
  ...
  test "merges consumer events by consumer_id" do
    pid = start_supervised!({PosthogReporter, [publish_interval: 10, name: Module.concat(__MODULE__, PosthogReporter)]})
    Req.Test.allow(Sequin.Posthog, self(), pid)
