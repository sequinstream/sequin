defmodule Sequin.SlotMessageProducerTest do
  ...
  describe "SlotMessageProducer with persisted messages" do
    setup do
      ...
      start_supervised!({SlotMessageProducer, consumer_id: consumer.id, test_pid: self()})
    ...
    test "messages from disk are loaded on init and then produce_messages returns them", %{
      ...
      assert {:ok, delivered} = SlotMessageProducer.produce(consumer.id, 2, self())
      assert length(delivered) == 2
      assert Enum.all?(delivered, fn msg -> msg.id in [msg1.id, msg2.id] end)

      assert SlotMessageProducer.peek(consumer.id).payload_size_bytes > 0
    end
    ...
    test "producing and acking persisted messages deletes them from disk", %{consumer: consumer} do
      ...
      assert {:ok, delivered} = SlotMessageProducer.produce(consumer.id, 10, self())
      assert length(delivered) == 2

      ack_ids = Enum.map(delivered, & &1.ack_id)
      {:ok, 2} = SlotMessageProducer.messages_succeeded(consumer.id, ack_ids)
    ...
    test "putting messages with persisted group_ids will upsert to postgres", %{
      ...
      :ok = SlotMessageProducer.put_messages(consumer.id, [new_message])
  ...
  describe "SlotMessageProducer message handling" do
    setup do
      consumer = ConsumersFactory.insert_sink_consumer!()
      event_consumer = ConsumersFactory.insert_sink_consumer!(message_kind: :event)
      start_supervised!({SlotMessageProducer, consumer_id: consumer.id, test_pid: self()})
      start_supervised!({SlotMessageProducer, consumer_id: event_consumer.id, test_pid: self()})
    ...
    test "puts, delivers, and acks in-memory messages", %{consumer: consumer} do
      ...
      :ok = SlotMessageProducer.put_messages(consumer.id, messages)

      # Retrieve messages
      {:ok, delivered} = SlotMessageProducer.produce(consumer.id, 2, self())
      assert length(delivered) == 2
      ...
      {:ok, []} = SlotMessageProducer.produce(consumer.id, 2, self())
    end
    ...
    test "deletes failed then succeeded messages from postgres", %{consumer: consumer} do
      ...
      :ok = SlotMessageProducer.put_messages(consumer.id, [message])

      # Deliver message
      {:ok, [delivered]} = SlotMessageProducer.produce(consumer.id, 1, self())
      ...
      :ok = SlotMessageProducer.messages_failed(consumer.id, [meta])
      ...
      {:ok, [redelivered]} = SlotMessageProducer.produce(consumer.id, 1, self())
      {:ok, 1} = SlotMessageProducer.messages_succeeded(consumer.id, [redelivered.ack_id])
    ...
    test "deletes failed then already_succeeded messages from postgres", %{consumer: consumer} do
      ...
      # Redeliver and succeed the message
      {:ok, [redelivered]} = SlotMessageProducer.produce(consumer.id, 1, self())
      {:ok, 1} = SlotMessageProducer.messages_already_succeeded(consumer.id, [redelivered.ack_id])
    ...
    test "delivers and fails messages to disk", %{consumer: consumer} do
      ...
      :ok = SlotMessageProducer.messages_failed(consumer.id, metas)

      # Verify messages were persisted with updated metadata
      messages = SlotMessageProducer.peek_messages(consumer.id, 2)
    ...
    test "when a message fails, non-persisted messages of the same group_id are persisted", %{consumer: consumer} do
      ...
      :ok = SlotMessageProducer.put_messages(consumer.id, [message1, message2])
    ...
    test "if the pid changes between calls of produce_messages, produced_messages are available for deliver", %{
      ...
      {:ok, delivered} = SlotMessageProducer.produce(consumer.id, 2, Factory.pid())
      assert length(delivered) == 2
    end
    ...
    test "messages with nil group_ids are not blocked when other nil group_id messages are persisted", %{
      ...
      :ok = SlotMessageProducer.put_messages(consumer.id, [message1])
      {:ok, [delivered]} = SlotMessageProducer.produce(consumer.id, 1, self())
    ...
    test "persists all messages when consumer is disabled", %{consumer: consumer} do
      ...
      :ok = SlotMessageProducer.put_messages(consumer.id, [initial_message])
      assert_receive {:put_messages_done, ^consumer_id}, 1000
      ...
      :ok = SlotMessageProducer.consumer_updated(disabled_consumer)

      # This will cause an async flush, wait for it by making any other call
      SlotMessageProducer.peek_messages(consumer.id, 1)
      ...
      :ok = SlotMessageProducer.put_messages(consumer.id, new_messages)
      assert_receive {:put_messages_done, ^consumer_id}, 1000
    ...
    test "duplicate messages don't accumulate payload size", %{consumer: consumer} do
      ...
      # Ack the message
      {:ok, 1} = SlotMessageProducer.messages_succeeded(consumer.id, [delivered.ack_id])
    ...
    test "messages_succeeded_returning_messages returns the acked messages", %{consumer: consumer} do
      ...
      ack_ids = Enum.map(delivered, & &1.ack_id)
      {:ok, returned_messages} = SlotMessageProducer.messages_succeeded_returning_messages(consumer.id, ack_ids)
  ...
  describe "SlotMessageProducer table reader batch handling" do
    setup do
      consumer = ConsumersFactory.insert_sink_consumer!()
    ...
    test "puts batch and reports on batch progress", %{consumer: consumer} do
      ...
      # Put messages in store
      :ok = SlotMessageProducer.put_table_reader_batch(consumer_id, messages, "test-batch-id")

      # Retrieve messages
      {:ok, [delivered]} = SlotMessageProducer.produce(consumer_id, 1, self())

      assert {:ok, ["test-batch-id"]} == SlotMessageProducer.unpersisted_table_reader_batch_ids(consumer_id)

      # For acks
      {:ok, 1} = SlotMessageProducer.messages_succeeded(consumer_id, [delivered.ack_id])
      ...
      assert {:ok, []} == SlotMessageProducer.unpersisted_table_reader_batch_ids(consumer_id)
    end
  end

  describe "SlotMessageProducer wal cursors" do
    setup do
      consumer = ConsumersFactory.insert_sink_consumer!()
    ...
    test "min_wal_cursor raises on ref mismatch", %{consumer: consumer} do
      pid = GenServer.whereis(SlotMessageProducer.via_tuple(consumer.id))
      ref = make_ref()
      :ok = SlotMessageProducer.set_monitor_ref(pid, ref)

      assert SlotMessageProducer.min_unpersisted_wal_cursor(consumer.id, ref) == nil

      assert_raise(InvariantError, fn ->
        SlotMessageProducer.min_unpersisted_wal_cursor(consumer.id, make_ref())
      end)
    end
  end
end
