defmodule Sequin.TransformsTest do
  ...
  describe "path_transform/1" do
    ...
    test "transforms a consumer message with sink path" do
      message = ConsumersFactory.consumer_message()
      path_transform = ConsumersFactory.path_transform(path: "metadata.sink")
      consumer = %SinkConsumer{transform: %Transform{transform: path_transform}}

      result = Transforms.Message.to_external(consumer, message)
      assert result == message.data.metadata.sink |> Map.from_struct() |> Sequin.Map.stringify_keys()
    end

    test "transforms a consumer message with sink name path" do
      message = ConsumersFactory.consumer_message()
      path_transform = ConsumersFactory.path_transform(path: "metadata.sink.name")
      consumer = %SinkConsumer{transform: %Transform{transform: path_transform}}

      result = Transforms.Message.to_external(consumer, message)
      assert result == message.data.metadata.sink.name
    end
